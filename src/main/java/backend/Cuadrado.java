...
//APLICANDO CONCEPTO DE HERENCIA 
public class Cuadrado extends Figura {
    ...
    public String guardarBDD() {
        try {
            ...
            Document circuloDocument = new Document("tipo", "Cuadrado")
                    .append("lado", this.lado)
   ...
   @Override
    public List<Document> consultarAreas() {
        try (MongoClient mongoClient = MongoClients.create("mongodb://localhost:27017")) {
            ...
            try (MongoCursor<Document> cursor = areasCollection.find(Filters.eq("tipo", "Cuadrado")).iterator()) {
            ...
            System.err.println("Error al obtener áreas de cuadrado: " + e.getMessage());
    ...
    public String eliminarAreas() {
        try {
            ...
            areasCollection.deleteMany(Filters.eq("tipo", "Cuadrado"));

            mongoClient.close();

            return "Áreas de cuadrado eliminadas de la base de datos.";
        } catch (Exception e) {
            return "Error al eliminar las áreas de cuadrado de la base de datos: " + e.getMessage();
        }
    }
}
